...
2. 던전 생성, 아이템 배치, 적 배치 — 전부 `engine/rng.py`의 시드 RNG. **층마다 root seed에서
   layout / items / enemies 독립 스트림을 파생한다** (`Rng.derive(namespace, floor)`). 파생은
   부모를 소비하지 않으므로 한 스트림의 소비량 변화가 다른 스트림이나 **이후 층**을 흔들지
   않는다 — 적 조합만 바꾼 동일 시드 비교가 **실제 통제 실험**이 된다. 생성 순서는
   구조 → 아이템 예약 → 적 배치다(적이 아이템 칸을 피한다). 파생에 `hash()`를 쓰면
   프로세스별 salt로 결정론이 깨지므로 `blake2s`를 쓴다. → 설계 `evt_5c9d0278`
   (이 분리가 없을 때 적 1마리 제거가 아이템·후속 층까지 바꿔 밸런스 비교가 무의미했다.)
...
- 공개 URL **4종**을 상호 링크: **플레이 / GitHub 저장소 / 정적 메모리 페이지(비인증) /
  AiAkiv sample 프로젝트(가입, 라이브 읽기전용)**.
  이것이 "이 히스토리가 이 게임을 만들었다"의 폐루프.
  (원래 3종이었다 — Public read 기능 부재로 메모리 링크가 둘로 갈라졌다.
  → [09_risks_checklist.md](09_risks_checklist.md) R1)
- 정적 메모리 페이지는 그래프를 HTML로 export해 GitHub Pages에 올린다 (Phase 5).
