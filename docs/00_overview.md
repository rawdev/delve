...
저장하고 읽기전용으로 공개한다. 데모의 주인공은 게임이 아니라 개발 궤적이다.**

방문자는 그래프에 직접 질문을 던져 "여러 사람·여러 AI가 함께 쌓은 공유 메모리"가
무엇을 할 수 있는지 체감한다. git 히스토리는 **무엇이** 바뀌었는지만 보여주지만,
메모리는 각 결정의 **왜**(어떤 대안을 검토하고 왜 버렸는지)를 보여준다.

> **공개 경로는 두 층이다** — 비인증 방문자는 **정적 페이지**에서 궤적을 *읽고*,
> AiAkiv 가입 유저는 **sample 프로젝트**(라이브 읽기전용)로 그래프를 *심문*한다.
> 원래는 단일 Public read를 전제했으나 그 기능이 없음이 확인되어 조정했다.
