...
>
> **다만 Phase 2로 넘어가기 전에 두 가지가 남아 있다:**
> - **Phase 0 콘솔 게이트 4건** — 특히 🚨 **Public read 확인**. 안 열리면 계획 자체가
>   무너지므로 코드 확장보다 먼저다 (R1).
> - **Phase 1 잔여 4건** — 리뷰 `evt_d38f1dc8`가 남긴 동시성·세션 수명·API 테스트.
>   (provenance 소급 복구는 완료.)
...
- [ ] **동일 `game_id` 동시 요청 직렬화** — 동기 엔드포인트가 공유 `GameState`/`Rng`를
      잠금 없이 변경한다. 경합 시 턴·전투·RNG 소비가 어긋난다 (게임별 Lock)
- [ ] **인메모리 세션 수명/용량 제한** — `_games`에서 상태가 제거되지 않는다. 공개 배포 전
      최소 방어선 (dead/won 정리, TTL, 최대 세션 수)
- [ ] **HTTP/API 계약 테스트** — 생성→조회→행동, 409/422, 미탐색 타일·비가시 적 비노출
- [ ] untracked 로컬 설정(`.codex/`, `AGENTS.md`) 추적 여부 결정 — 공개 저장소 유입 방지
